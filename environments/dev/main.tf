...
module "kms_integration" {
  ...
  keys = {
    msk = {
      ...
      # dev gets destroyed/recreated often — the 30-day default just leaves more orphaned
      # pending-deletion keys sitting around than a fast-iteration environment needs (this
      # does not speed up redeploys, unlike the Redis secret's recovery window below — the
      # alias frees immediately on destroy regardless of this value).
...
module "kms_nrt_processing" {
  ...
  keys = {
    elasticache = {
      ...
      deletion_window_in_days = 7 # dev-only fast-iteration override — see kms_integration's msk key above for why
    }
    flink-lambda-logs = {
...
module "elasticache" {
  ...
  # dev gets destroyed/recreated often — a 30-day recovery window (AWS's default) would block
  # re-creating the auth-token secret with the same name on the next apply, the way it just did.
